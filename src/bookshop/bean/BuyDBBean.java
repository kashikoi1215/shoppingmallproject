...
public class BuyDBBean {
	...
	public List<String> getAccount(){
		...
		try {
			...
			pstmt = conn.prepareStatement("select * from back");
			...
			while(rs.next()) {
				String account = new String(rs.getString("account") + " "
					+ rs.getString("back") + " " + rs.getString("name"));
	...
	public void insertBuy(List<CartDataBean>) lists,
		...
		try {
			...
			if(rs.getLong(1) > 0) {
				...
				if(compareDate.equals(maxDate)) {
					if((Integer.parseInt(number)+1) < 10000) {
						buyId = Long.parseLong(maxDate + (Integer.parseInt(number)
								+1 +10000));
					}else {
			...
			for(int i = 0; i < lists.size(); i++) {
				...
				nowCount = (short)(rs.getShort(1) -1); // 실무에서는 구매수량을 뺄 것
